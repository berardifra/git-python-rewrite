...
# argparse is a nice library but its interface may not be the most intuitive ever
argparser = argparse.ArgumentParser(description="The stupidest version control")

# At this point we only need to declare that our CLI will use some, and that all invocation will actually require one
...
# Obviously, the first Git command in chronological and logical order is "git init", so we’ll begin by creating "wyag init".
# A git repository is made of two things: a “work tree”, where the files meant to be in version control live, 
# and a “git directory”, where Git stores its own data. 
...
class GitRepository (object):
    ...
    def __init__(self, path, force=False):
        self.worktree = path
        self.gitdir = os.path.join(path, ".git")            # address of (path + .git)
